<?php
	class BoardDao{
		...
		public function insertData($writer,$title,$content){
			try{
				$ps = $this -> db -> prepare("insert into board(writer,title,content,regtime,hits) value(:writer,:title,:content,now(),0)");
		...
		public function updateData($num,$writer,$title,$content){
			try{
				$ps = $this -> db -> prepare("update board set writer=:writer, title=:title, content=:content, regtime=now() where num=:num");
				$regtime = date("Y-m-d H:i:s");
				$ps -> bindValue(":writer",$writer,PDO::PARAM_STR);
				$ps -> bindValue(":title",$title,PDO::PARAM_STR);
				$ps -> bindValue(":content",$content,PDO::PARAM_STR);
