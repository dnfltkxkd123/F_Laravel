...
<script>
     function loginCheck(check){
        if(check){
            location.href='openBoard_register_form.php?url=<?=$url?>&page=<?=$currentPage?>&num=<?=$num?>&form=insert';
        }else{
            alert('로그인 해주세요!!');
        }
      }
   ...
   <div class="container">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <?php foreach($onePageList as $row): ?>
          ...
          ?>
          <tr onclick="location.href='openBoard_page.php?num=<?=$row['num']?>&page=1'" style='cursor:pointer;'>
            <td><?= $row['title']?></td>
    ...
    <div style='text-align: center;'>
      ...
      <a href='openBoard.php?page=<?=$currentPage-5?>'>< </a>
      <?php endif?>

      <?php for($i=$firstLink ; $i<=$lastLink ; $i++):?>
        <a href='openBoard.php?page=<?=$i?>'><?=$i?> </a>
      <?php endfor?>
