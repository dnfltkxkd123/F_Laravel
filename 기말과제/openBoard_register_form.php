<?php
  ...
  if($form == 'insert'){
    $formText = '글작성';
    $url = 'openBoard_register.php?page='.$currnetPage;
    logoutBack(bdUrl('openBoard.php',$num,$currnetPage));
  }else if($form == 'update'){
    $formText = '글수정';
    $url = 'openBoard_update.php?page='.$currnetPage.'&num='.$num;
    logoutBack(bdUrl('openBoard_page.php',$num,$currnetPage));
  }
  logoutBack(bdUrl('openBoard.php',$num,$currnetPage));
?>
<!DOCTYPE html>
...
?>
  ...
  <div class='contain'>
    <h1><?=$formText?></h1>
    <form name="nse" action="<?=$url?>" method="post" >
